...
function M.lsp(picker, inlay_hint_supported)
	...
	if vim.fn.exists(':RustLsp') then
		...
		vim.keymap.set("n", "gh", "<cmd>RustLsp renderDiagnostic<cr>", { desc = "RustLsp render diagnostic" })
	end
end
...
function M.snacks_picker_keymaps(picker)
	vim.keymap.set("n", "<leader>ff", picker.files, { desc = "Find file" })

	-- Ctrl-P make it be the same
	vim.keymap.set("n", "<C-p>", picker.files, { desc = "Find file (in git repository)" })
	...
	vim.keymap.set("n", "<leader>fg", picker.git_diff, { desc = "git - modified files" })
	vim.keymap.set("n", "<leader>;", picker.buffers, { desc = "Telescope search buffers" })
	...
	vim.keymap.set("n", "<leader>b", picker.buffers, { desc = "Telescope search buffers" })
...
---@param gitbrowse snacks.gitbrowse
function M.snacks_gitbrowse_keymaps(gitbrowse)
	vim.keymap.set("n", "<leader>go", gitbrowse.open, { desc = "Open current repository in browser" })
end
