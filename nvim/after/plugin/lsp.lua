...
local should_format_with_client = function(client)
	return client.name ~= "tsserver" and client.name ~= "jsonls" and client.name ~= "copilot" and client.name ~= "volar"
end
...
local on_attach = function(client, bufnr)
	...
	if client.supports_method("textDocument/formatting") then
		...
		-- NOTE: handled by conform plugin
		-- vim.api.nvim_create_autocmd("BufWritePre", {
		-- 	pattern = "*",
		-- 	group = augroup,
		-- 	callback = function(ev)
		-- 		local buf_name = ev.file
		--
		-- 		-- NOTE: selfie.c, beator.c and others are huge. Don't format them
		-- 		local is_selfie_dir = string.match(buf_name, "selfie")
		--
		-- 		if not is_selfie_dir then
		-- 			lsp_format()
		-- 		end
		-- 	end,
		-- })
